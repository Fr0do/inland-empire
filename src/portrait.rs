...
pub fn render_character(ch: &Character) -> String {
    ...
    // ── Head ──
    let head_l = if has_hat {
        slot_fill(EquipSlot::Hat, "║")
    } else {
        "│".dimmed().to_string()
    };
    let head_r = if has_hat {
        slot_fill(EquipSlot::Hat, "║")
    } else {
        "│".dimmed().to_string()
    };
    ...
    let leg_bl = if has_pants {
        slot_fill(EquipSlot::Pants, "└──")
    } else {
        "└──".dimmed().to_string()
    };
    let leg_br = if has_pants {
        slot_fill(EquipSlot::Pants, "──┘")
    } else {
        "──┘".dimmed().to_string()
    };
    ...
    let (shoe_l, shoe_r) = if has_shoes {
        (
            slot_fill(EquipSlot::Shoes, "╔══╧╗"),
            slot_fill(EquipSlot::Shoes, "╔╧══╗"),
        )
    } else {
        ("┌──┴┐".dimmed().to_string(), "┌┴──┐".dimmed().to_string())
    };
    let shoe_bl = if has_shoes {
        slot_fill(EquipSlot::Shoes, "╚═══╝")
    } else {
        "└───┘".dimmed().to_string()
    };
    let shoe_br = if has_shoes {
        slot_fill(EquipSlot::Shoes, "╚═══╝")
    } else {
        "└───┘".dimmed().to_string()
    };
    ...
    let hat_lbl = slot_name(ch, EquipSlot::Hat)
        .map(|n| slot_color(EquipSlot::Hat, &n))
        .unwrap_or("(none)".dimmed().to_string());
    let jacket_lbl = slot_name(ch, EquipSlot::Jacket)
        .map(|n| slot_color(EquipSlot::Jacket, &n))
        .unwrap_or("(none)".dimmed().to_string());
    let shirt_lbl = slot_name(ch, EquipSlot::Shirt)
        .map(|n| slot_color(EquipSlot::Shirt, &n))
        .unwrap_or("(none)".dimmed().to_string());
    let pants_lbl = slot_name(ch, EquipSlot::Pants)
        .map(|n| slot_color(EquipSlot::Pants, &n))
        .unwrap_or("(none)".dimmed().to_string());
    let shoes_lbl = slot_name(ch, EquipSlot::Shoes)
        .map(|n| slot_color(EquipSlot::Shoes, &n))
        .unwrap_or("(none)".dimmed().to_string());
    let acc_lbl = slot_name(ch, EquipSlot::Accessory)
        .map(|n| slot_color(EquipSlot::Accessory, &n))
        .unwrap_or("(none)".dimmed().to_string());

    // ── Health/Morale bars ──
    let hearts: String = "❤".repeat(ch.health as usize).red().to_string()
        + &"♡"
            .repeat((ch.max_health - ch.health) as usize)
            .bright_black()
            .to_string();
    let morale_bar: String = "◆".repeat(ch.morale as usize).magenta().to_string()
        + &"◇"
            .repeat((ch.max_morale - ch.morale) as usize)
            .bright_black()
            .to_string();
