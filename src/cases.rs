...
pub fn all_cases() -> Vec<Case> {
    vec![
        ...
        Case {
            ...
            description: "Chemistry is a tool like any other. Use four different substances.".into(),
...
pub fn check_case_progress(case: &Case, ch: &Character) -> (usize, usize) {
    match &case.objective {
        Objective::CheckCount(target) => {
            (ch.check_history.len().min(*target), *target)
        }
        Objective::SkillChecks(skill, target) => {
            let count = ch.check_history.iter()
        ...
        Objective::ReachLevel(target) => {
            (ch.level.min(*target) as usize, *target as usize)
        }
        Objective::InternalizeThoughts(target) => {
            use crate::character::ThoughtPhase;
            let count = ch.thoughts.iter()
        ...
        Objective::UseSubstances(target) => {
            ...
            for e in &ch.active_effects { seen.insert(e.substance); }
            // Also count substances in inventory as "encountered"
            for s in ch.inventory.keys() { seen.insert(*s); }
...
fn best_success_streak(ch: &Character) -> usize {
    ...
    for r in &ch.check_history {
        if r.success { current += 1; best = best.max(current); }
        else { current = 0; }
    }
    best
}
