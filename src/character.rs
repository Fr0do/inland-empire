...
pub const ARCHETYPES: &[Archetype] = &[
    Archetype {
        name: "Thinker",
        description: "High Intellect. Analyzes everything, misses nothing logical.",
        attributes: [
            (Attribute::Intellect, 5),
            (Attribute::Psyche, 3),
            (Attribute::Physique, 2),
            (Attribute::Motorics, 2),
        ],
    },
    Archetype {
        name: "Sensitive",
        description: "High Psyche. Feels the code, understands the user.",
        attributes: [
    ...
    Archetype {
        name: "Bruiser",
        description: "High Physique. Brute-forces through problems.",
        attributes: [
            (Attribute::Intellect, 2),
            (Attribute::Psyche, 2),
            (Attribute::Physique, 5),
            (Attribute::Motorics, 3),
        ],
    },
    Archetype {
        name: "Operator",
        description: "High Motorics. Precise, fast, elegant.",
        attributes: [
            (Attribute::Intellect, 2),
            (Attribute::Psyche, 2),
            (Attribute::Physique, 3),
            (Attribute::Motorics, 5),
        ],
    },
    Archetype {
        name: "Generalist",
        description: "Balanced across all attributes.",
        attributes: [
            (Attribute::Intellect, 3),
            (Attribute::Psyche, 3),
            (Attribute::Physique, 3),
...
impl Character {
    pub fn new(name: String, archetype_name: &str, signature: Option<Skill>) -> Self {
        let arch = ARCHETYPES
            .iter()
            .find(|a| a.name.eq_ignore_ascii_case(archetype_name))
            .unwrap_or(&ARCHETYPES[4]);
    ...
    pub fn effective_skill(&self, skill: Skill) -> i8 {
        ...
        let thought_bonus: i8 = self
            ...
            .sum();
        let signature_bonus: i8 = if self.signature_skill == Some(skill) {
            1
        } else {
            0
        };
        let time_bonus = TimeOfDay::current()
            .modifiers()
            .get(&skill)
            .copied()
            .unwrap_or(0);
        let substance_bonus: i8 = self
            .active_effects
            .iter()
            .filter_map(|e| e.skill_modifiers.get(&skill))
            .sum();
        let equipment_bonus: i8 = self
            .loadout
            .total_modifiers()
            .get(&skill)
            .copied()
    ...
    pub fn add_xp(&mut self, amount: u32) -> Option<(u32, Substance)> {
        ...
        if self.xp >= needed {
            ...
            self.journal.push(journal::generate_entry(
                self.genre,
                EntryType::LevelUp,
                &vars,
            ));
    ...
    pub fn last_failed_white_check(&self, skill: Skill) -> Option<&CheckRecord> {
        self.check_history
            .iter()
            .rev()
            .find(|r| r.skill == skill && !r.success && r.check_color == CheckColor::White)
    }
    ...
    pub fn equip_thought(&mut self, mut thought: Thought) -> Result<(), String> {
        ...
        for (skill, val) in &thought.skill_modifiers {
            if *val > 0 {
                research_mods.insert(*skill, -1i8);
            }
        }
        thought.research_modifiers = research_mods;
        thought.phase = ThoughtPhase::Researching {
            checks_remaining: 5,
        };
    ...
    pub fn use_substance(&mut self, substance: Substance) -> Result<String, String> {
        ...
        if let Some(required) = substance.required_thought() {
            ...
            if !has_thought {
                return Err(format!(
                    "You don't know about {} yet. You need to think deeper...",
                    substance
                ));
            }
        }
        ...
        if count == 0 {
            return Err(format!("No {} in inventory.", substance));
        }
        self.inventory.insert(substance, count - 1);
        if count - 1 == 0 {
            self.inventory.remove(&substance);
        }
        let info = substance.info();
        if info.health_restore > 0 {
            self.heal(info.health_restore as u8);
        }
        if info.morale_restore > 0 {
            self.restore_morale(info.morale_restore as u8);
        }
    ...
    pub fn record_check(&mut self, record: CheckRecord) -> Option<(u32, Substance)> {
        ...
        if record.roll.0 == 6 && record.roll.1 == 6 {
            self.journal.push(journal::generate_check_entry(
                self.genre,
        ...
        for thought in &mut self.thoughts {
            if let ThoughtPhase::Researching {
                ref mut checks_remaining,
            } = thought.phase
            {
    ...
    pub fn load_active() -> Result<Self, String> {
        ...
        let name = std::fs::read_to_string(&active_path)
            .map_err(|_| "No active character. Run: ie new <name>".to_string())?;
        Self::load(name.trim())
    }
...
fn profile_path(name: &str) -> PathBuf {
    data_dir().join("profiles").join(format!("{}.json", name))
}
